...
export default function ResumenEventoPage() {
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          if (userData?.rol === "padre") {
            ...
            const hijosList = hijosSnap.docs.map(d => {
              const hijoData = d.data();
              return {
                id: d.id,
                padre_id: user.uid,
                nombre: hijoData.nombre || "Hijo sin nombre",
                apellidos: hijoData.apellidos || null,
                equipoId: hijoData.equipoId || null,
              } as HijoData;
            });
            ...
            return Promise.all(
              jugadoresArray.map(async (j: any) => {
                return {
                  ...
                  padre_id: j.padre_id || null,
                  tipo: j.tipo || tipoDefault,
                  estado: j.estado || "pendiente",
                  nombre: j.nombre || null,
                  nombre_hijo: j.nombre_hijo || null,
                  apellidos: j.apellidos || null,
                  apellidos_hijo: j.apellidos_hijo || null,
                  telefono: j.telefono || null,
                  telefono_padre: j.telefono_padre || null,
            ...
            return Promise.all(
              transporteArray.map(async (t: any) => {
                return {
                  ...
                  nombre_padre: t.nombre_padre || "Padre Desconocido",
                  nombre_hijo: t.nombre_hijo || "Hijo no especificado",
                  plazas: typeof t.plazas === 'number' ? t.plazas : 0,
    ...
    const actualizarListaJugadores = (lista: JugadorConv[]): JugadorConv[] => {
      ...
      return lista.map(jOriginal => {
        const jugadorActualizado: JugadorConv = {
          ...
          tipo: jOriginal.tipo || "reserva",
          estado: jOriginal.estado || "pendiente",
          nombre: jOriginal.nombre || null,
          nombre_hijo: jOriginal.nombre_hijo || null,
          apellidos: jOriginal.apellidos || null,
          apellidos_hijo: jOriginal.apellidos_hijo || null,
          telefono: jOriginal.telefono || null,
          telefono_padre: jOriginal.telefono_padre || null,
  ...
  // Lógica: la respuesta se puede modificar hasta 12 horas antes del partido
  let puedePadreModificarRespuesta = false;
  if (isPartido && evento?.fecha && evento?.horaInicio) {
    const fechaHoraEvento = dayjs(`${evento.fecha} ${evento.horaInicio}`, "DD-MM-YYYY HH:mm");
    puedePadreModificarRespuesta = dayjs().isBefore(fechaHoraEvento.subtract(12, "hour"));
  }
  ...
  const puedeMostrarBotonModificarRespuesta = (
    rol === "padre" &&
    isPartido &&
    hijoConvocado &&
    puedePadreModificarRespuesta &&
    (estadoHijo === "confirmado" || estadoHijo === "rechazado")
  );
        ...
        <Stack spacing={3}>
          ...
          {rol === "padre" && isPartido && alertaConvocatoriaPadre && (
            <Stack spacing={2} p={2} bgcolor={theme.palette.mode === 'dark' ? theme.palette.grey[800] : theme.palette.grey[100]} borderRadius={theme.shape.borderRadius}>
              ...
              {(estadoHijo === "pendiente" || estadoHijo === "reserva" || mostrarRespuestas) && puedePadreModificarRespuesta && hijoConvocado && (
              ...
              {puedeMostrarBotonModificarRespuesta && !mostrarRespuestas && (
                <Button
                  variant="text"
                  size="small"
                  color="primary"
                  sx={{ mt: 1, alignSelf: "flex-end" }}
                  onClick={() => setMostrarRespuestas(true)}
                >
                  Modificar respuesta
                </Button>
              )}
          ...
          {rol === "entrenador" && isPartido && convocatoria && (
            <Stack spacing={2}>
              ...
              {((convocatoria.titulares || []).length === 0 && (convocatoria.reservas || []).length === 0) ? (
                ...
                <>
                  {[
                    { title: "Lista de convocados", players: convocatoria.titulares || [], icon: <ListAltIcon /> },
                    { title: "Lista de reservas", players: convocatoria.reservas || [], icon: <PeopleOutlineIcon /> }
                  ].map(section => (
                    section.players.length > 0 &&
                    <Accordion key={section.title} defaultExpanded={section.title === "Lista de convocados"}>
                        ...
                        <List dense disablePadding>
                          {section.players.map((j) => (
                            <ListItem key={j.jugador_id || Math.random()} divider sx={{ py: 1, '&:last-child': { borderBottom: 'none' } }}>
                              <ListItemText
                                primary={
                                  (j.nombre_hijo || j.nombre || "Jugador Desconocido") +
                                  " " +
                                  (j.apellidos_hijo || j.apellidos || "")
                                }
                                secondary={
                                  <>
                                    {j.telefono_padre || j.telefono || "Sin teléfono"}
                                    {j.estado === "rechazado" && (j.motivo || j.motivo === "") && (
